...
export default function MenuPage() {
  ...
  const handleCreateItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        externalImageUrl: itemFormData.externalImageUrl || undefined
      };
  ...
  const handleUpdateItemInfo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only send externalImageUrl if it has changed or was explicitly cleared
      if ('externalImageUrl' in (selectedItem as any)) {
        payload.externalImageUrl = selectedItem.externalImageUrl || null;
      }
  ...
  const handleItemImageUpload = async (file: File) => {
    ...
    try {
      ...
      const res = await api.post(`/upload/menu-item/${selectedItem.id}/image`, formData, {
  ...
  const handleRemoveItemImage = async () => {
    ...
    setConfirmDialog({
      ...
      message: 'Remove the uploaded image from this dish?',
      onConfirm: async () => {
        try {
          await api.delete(`/upload/menu-item/${selectedItem.id}/image`);
          setSuccess('Dish image removed.');
          fetchSingleItem(selectedItem.id);
          setTimeout(() => setSuccess(null), 3000);
        } catch (err) {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        {(() => {
                          const imgSrc = item.imagePublicId ? item.imageUrl : (item.externalImageUrl || item.imageUrl);
                          return imgSrc ? (
                ...
                <div className="p-6">
                  ...
                  {detailTab === 'INFO' && (
                    <form onSubmit={handleUpdateItemInfo} className="space-y-4">
                      ...
                      {/* Dish Image Preview */}
                      {(() => {
                        const displaySrc = selectedItem.imagePublicId ? selectedItem.imageUrl : (selectedItem.externalImageUrl || selectedItem.imageUrl);
                        return displaySrc ? (
                          <div className="relative w-32 h-32 rounded-xl border border-slate-700 bg-slate-950 overflow-hidden">
                            {uploadLoading && (
                            ...
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={displaySrc} alt={selectedItem.name} className="w-full h-full object-cover" />
                            {selectedItem.imagePublicId && (
                              <span className="absolute top-1 left-1 bg-emerald-500/80 text-[8px] font-bold text-white px-1.5 py-0.5 rounded">Uploaded</span>
                            )}
                          </div>
                        ) : null;
                      })()}
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Dish Photo Upload</label>
                        <input
                          type="file"
                          ref={fileInputRef}
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 text-xs font-bold bg-slate-800 hover:bg-slate-700 text-white rounded-lg transition-all disabled:opacity-50"
                          >
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">External Image URL</label>
                        <input
                          type="url"
                          placeholder="https://example.com/dish.jpg"
                          value={selectedItem.externalImageUrl || ''}
                          onChange={(e) => setSelectedItem({ ...selectedItem, externalImageUrl: e.target.value || null })}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    type="url"
                    placeholder="https://example.com/dish.jpg"
                    value={itemFormData.externalImageUrl}
                    onChange={(e) => setItemFormData({ ...itemFormData, externalImageUrl: e.target.value })}
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2.5 text-xs text-white placeholder-slate-600 outline-none focus:border-emerald-500 transition-all"
                  />
                </div>
              </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-zoomIn">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => !confirmLoading && setConfirmDialog(prev => ({ ...prev, isOpen: false }))}
                  disabled={confirmLoading}
                  className="flex-1 py-2 text-xs font-bold border border-slate-800 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-white transition-all disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-2 text-xs font-black bg-red-500 hover:bg-red-600 text-white rounded-lg transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                >
