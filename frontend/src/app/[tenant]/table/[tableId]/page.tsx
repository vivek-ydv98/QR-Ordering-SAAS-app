      ...
      <header className="sticky top-0 z-30 flex flex-col w-full border-b border-slate-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-10 h-10 rounded-full bg-primary/10 text-primary font-bold text-lg overflow-hidden">
              {tenant.logoUrl ? (
              ...
              <div className="flex flex-col gap-4">
                {catItems.map((item) => (
                  ...
                  >
                    ...
                    {/* Dish Image */}
                    {(item.imagePublicId ? item.imageUrl : item.externalImageUrl || item.imageUrl) && (
                      <div className="relative w-24 h-24 rounded-lg overflow-hidden bg-slate-100 flex-shrink-0">
                        <img
                          src={item.imagePublicId ? item.imageUrl : (item.externalImageUrl || item.imageUrl!)}
                          alt={item.name}
